							...
							<div class="card-body">
								<div class="border p-4 rounded">
									...
									<div class="row mb-3">
										...
										<div class="col-sm-9">
											<input type="text" name="page_hedding" value="{{ $page->page_hedding }}" class="form-control" id="inputEnterYourName" placeholder="Enter Page Hedding">
										</div>
									</div>
									<div class="row mb-3">
										<label for="inputEnterYourName" class="col-sm-3 col-form-label">Page Title</label>
										<div class="col-sm-9">
											<input type="text" name="page_title" value="{{ $page->page_title }}" class="form-control" id="inputEnterYourName" placeholder="Enter Page Title">
										</div>
									</div>
									<div class="row mb-3">
										...
										<div class="col-sm-9">
											<select name="category_id" id="category_id"  class="form-select category">
												...
												<option value="{{ $category->id }}" @if($category->id == $page->category_id) selected @endif>{{ $category->name }}</option>
												@endforeach
                                            </select>
										</div>
									</div>
										...
										<div class="col-sm-9">
											<select name="sub_category_id" id="sub_category_id" class="form-select">
                                               <option selected disabled>Select SubCategory</option>
												...
												<option value="{{ $subCategory->id }}" @if($subCategory->id == $page->subcategory_id) selected @endif>{{ $subCategory->name }}</option>
												@endforeach
                                            </select>
										</div>
									</div>
									...
									<div class="row mb-3">
										...
										<div class="col-sm-9">
											<input name="image" type="file" value="{{ $page->image }}" class="form-control" accept="image/*">
											<small class="text-muted">Upload image in jpg, jpeg, png format only.</small>
											@if($page->image)
												<img src="{{ asset( $page->image) }}" alt="page Image" class="img-thumbnail mt-2" style="max-width: 200px;">
											@endif
										</div>
									</div>
									<div class="row mb-3">
										...
										<div class="col-sm-9">
											<textarea id="editor" class="form-control" value="" name="content"> {{$page->content}}</textarea>
										</div>
									</div>
									<div class="row">
